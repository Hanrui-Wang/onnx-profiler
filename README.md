...
* `verbose` is set to `True` to display the number of multiplications in each layer; alternatively, you can also set it to `False` if you do not want any intermediate outputs.
* `reduction` is set to `np.sum` to sum up the computations in all layers. If you want to obtain the computations in all layers (as a list), you can then set this argument to `None`.
